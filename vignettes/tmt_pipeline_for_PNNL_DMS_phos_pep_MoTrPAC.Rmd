...
vignette: >
  %\VignetteIndexEntry{tmt_pipeline_for_PNNL_DMS_phos_pep_MoTrPAC}
...
x <- quant_cross_tab %>%
  ...
  mutate(Peptide = gsub(".\\.(.*)\\..","\\1",Peptide, perl=T)) %>%
  mutate(Peptide = gsub("(\\w)\\*","\\L\\1",Peptide, perl=T)) %>%
  mutate(SiteID = sub("-", "_", SiteID)) %>%
  mutate(ptm_peptide = paste0(SiteID, "-", Peptide))  %>%
  dplyr::rename(ptm_id = SiteID,
                peptide = Peptide,
                ascore = maxAScore,
                confidence = ConfidentSite) %>%
  select(-c(accession, RefSeq)) %>%
  select(ptm_peptide, ptm_id, peptide, ascore, confidence, everything())
head(x)
```
